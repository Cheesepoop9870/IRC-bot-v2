...
def decorator(deco):    
    spec = inspect.signature(deco)    
    if len(spec.parameters) > 1 or spec.varargs.kind is not inspect.Parameter.VAR_POSITIONAL or spec.keywords.kind is not inspect.Parameter.VAR_KEYWORD:
...
class cached_property:
    def __init__(self, func):
        self.func = func
        # Update the wrapper with the original function
        functools.update_wrapper(self.__get__, func) 
...
def split(text, delimeters):
    pattern = "|".join(re.escape(delimiter) for delimiter in delimeters)
