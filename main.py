...
port = 6697
channel_list = [
    "#cheesepoop9870", "#facility36", "#neutralzone", "#site22",
    "#Magnishideout"
]
...
ADMIN_USERS = {
    ...
    "my.poop.is.cheese",
    "illegal.food.combo",
    " stalking.your.sandbox",
    "site19.isnt.real.cant.hurt.you",
    "the.queen.of.foxes",
    "Magnileak",
}  # Add admin usernames/hosts here
ADMIN_USER_REGEX = {
    r"(\w+!)?(uid692117|theword987)@(stalking\.your\.sandbox|my\.poop\.is\.cheese|illegal\.food\.combo)$",
    r"(\w*!)?(uid536230)@(the\.queen\.of\.(the\.)?foxes)$",
    r"(\w+!)?(Felds)@(the\.amyrlin\.seat|site19\.isnt\.real\.cant\.hurt\.you)$",
    r"(\w+!)?(uid714194|Magnileak)@(SCP-f5eupe\.tinside\.irccloud\.com|SCP-kin\.6dp\.29\.161\.IP|SCP-phc\.lrc\.149\.118\.IP|SCP-go5\.s65\.149\.118\.IP)$"
}
...
def handle_command(command,
    ...
    if command == "hello":
        ...
        if checkperms(full_host, sender):
            handle.write(
                f'QUIT :Quit command used by {sender} in channel {channel_debug}\r\n'
            )
            handle.flush()
            log.info(
                f'Quit command used by {sender} in channel {channel_debug}')
            sys.exit(0)
        else:
            send_message(
                channel_debug,
                'Sorry, you are not authorized to use this command. Action logged.'
            )
            log.warning(
                f'{sender} tried to use the quit command in channel {channel_debug}'
            )
    elif command == "clear":
        ...
        send_message(
            channel_debug,
            'List of Commands: https://scp-sandbox-3.wikidot.com/mandobot-commands'
        )
        ...
        if checkperms(full_host, sender):
            ...
            #time.sleep(3)
            log.debug(
                f"setup command used by {sender} in channel {channel_debug}")
        else:
            send_message(
                channel_debug,
                'Sorry, you are not authorized to use this command. Action logged.'
            ...
            log.warning(
                f'{sender} tried to use the setup command in channel {channel_debug}'
            )
    elif command == "!roll":
        try:
            ...
            #inf/0 check
            if commandargs2[0] in [
                    "inf",
                    "0",
                    "Inf",
                    "infinity",
            ] or commandargs2[1] in [
                    "inf",
                    "0",
                    "Inf",
                    "infinity",
                ...
                if commandargs2[0] == "0" or commandargs2[1] == "0":
                    send_message(
                        channel_debug,
                        f"{sender} rolled {commandargs2[0]}d{commandargs2[1]} and got "
                    )
                    ...
                    if rng == 1:
                        send_message(
                            channel_debug,
                            f" {sender} rolled {commandargs2[0]}d{commandargs2[1]} and got {r.randint(-1000000000000000000000000000000000000000000000000000000, 1000000000000000000000000000000000000000000000000000000)}"
                        )
                        return
                    else:  #rng == 2
                        send_message(
                            channel_debug,
                            f"{sender} rolled {commandargs2[0]}d{commandargs2[1]} and got {infindex[r.randint(0, len(infindex)-1)]} "
                        )
                    rng = 0
                    commandargs = ""
                    commandargs2 = []
                ...
                for i in range(int(commandargs2[0])):
                    commandargsoutput.append(r.randint(1,
                                                       int(commandargs2[1])))
        ...
        #add some 1d6 +1 funtionality
            if cflag_plus_roll == 0:
                send_message(
                    channel_debug,
                    f"{sender} rolled {commandargs2[0]}d{commandargs2[1]}: {commandargs2[len(commandargs2)-1]} Total: {sum(commandargsoutput)}"
                )
            else:
                send_message(
                    channel_debug,
                    f"{sender} rolled {commandargs2[0]}d{commandargs2[1]}(+{commandargs3[1]}): {commandargs2[len(commandargs2)-1]} Total: {sum(commandargsoutput)}(+{commandargs3[1]}) = {sum(commandargsoutput, int(commandargs3[1]))}"
                )
            commandargs = ""
            commandargs2 = []
            commandargsoutput = []
            ...
            send_message(channel_debug,
                         'Invalid dice format. use 1d10 or similar')
            log.warning(
                f'{sender} tried to use the roll command in channel {channel_debug} but got an index error'
            )
        ...
        if "353" in response:  # 353 is the IRC code for names list
            ...
            send_message(
                channel_debug,
                'Error! if this happenes, tell cheese. Error string: Could not retrieve names'
            )
        ...
        if "#" not in args[0]:
            send_message(channel_debug,
                         f'{sender}: Invalid format. Use !join #channel')
        elif "#site19" in args[0] or "#site17" in args[0]:
            send_message(
                channel_debug,
                f'{sender}: Sorry, Mando-Bot is not authorized to join that channel. Action logged.'
            )  #dont need subsitute
            log.warning(
                f'{sender} tried to join {args[0]} in channel {channel_debug}')
        else:
        ...
        if checkperms(full_host, sender):
            ...
            log.warning(
                f'{sender} tried to use the leave command in channel {channel_debug}'
            )
        ...
        if not bool(list(search(args, num_results=1))[0]) or "/search?" in str(
                list(search(args, num_results=1))
            [0]):  # false, 1st result contains no results/404
            ...
            if ", title | , description |" in output_str or "/search?" in output_str:
                ...
                log.warning(
                    f'{sender} tried to use the google command in channel {channel_debug} but got no results'
                )
            else:
                send_message(channel_debug, f'{sender}: {output_str}')
        elif bool(list(search(args, num_results=1))
                  [0]):  # true, first result contains a 404 or error
            log.warning(
                f'{sender} tried to use the google command in channel {channel_debug} but got a 404 or error on first result'
            )
            try:
                ...
                if ", title | , description |" in output_str or "/search?" in output_str:
                    ...
                    send_message(
                        channel_debug,
                        f"{sender}: {output_str[0:len(output_str)-2]}")
            except Exception as e:
                send_message(
                    channel_debug,
                    f'Error! if this happenes, tell cheese. Error string: {e}')
                log.exception(f"Error: {e}")
        else:
            send_message(
                channel_debug,
                'Error! if this happens, tell cheese. Error string 424')
        ...
        if args[0] == "set":
            if args[1] == "debug":
                ...
                if checkperms(full_host, sender):
                    ...
                    debug("", "check")
                    log.info(
                        f'Debug mode set to {debug_flag} by {sender} in channel {channel_debug}'
                    )
                else:
                    send_message(
                        channel_debug,
                    ...
                    log.warning(
                        f'{sender} tried to use the debug command in channel {channel_debug}'
                    )
                ...
                if checkperms(full_host, sender):
                    ...
                    try:
                        if int(latest_range) > 5:
                            send_message(
                                channel_debug,
                                'Error! latest_range cannot be greater than 5')
                            log.warning(
                                f'{sender} tried to set latest_range to {latest_range} in channel {channel_debug}'
                            )
                            latest_range = 3
                        send_message(channel_debug,
                                     f'Latest search range = {latest_range}')
                    except ValueError:
                        send_message(channel_debug,
                                     'Error! latest_range must be an integer')
                        log.warning(
                            f'{sender} tried to set latest_range to {latest_range} in channel {channel_debug}'
                        )
                else:
                    ...
                    log.warning(
                        f'{sender} tried to use the latest_range command in channel {channel_debug}'
                    )
            elif args[1] == "cache":
                ...
                if checkperms(full_host, sender):
                    try:
                        ...
                        send_message(channel_debug,
                                     f'Cache duration = {args[2]}')
                        log.info(
                            f'Cache duration set to {args[2]} by {sender} in channel {channel_debug}'
                        )
                    except ValueError:
                        send_message(
                            channel_debug,
                            'Error! cache duration must be an integer')
                        log.warning(
                            f'{sender} tried to set cache duration to {args[2]} in channel {channel_debug}'
                        )
                    except Exception as e:
                        send_message(
                            channel_debug,
                    ...
                    log.warning(
                        f'{sender} tried to use the cache command in channel {channel_debug}'
                    )
        elif args[0] == "get":
            if args[1] == "debug":
                send_message(channel_debug, f'Debug mode = {debug_flag}')
        ...
        output = output_str.split(",")
        send_message(channel_debug,
                     f'{sender}: {output[r.randint(0, len(output)-1)]}')
        ...
        try:
            ...
            output4.append(output["rating"].split("(")[1]
                           [0:len(output["rating"].split("(")[1]) - 1])
            ...
            send_message(
                channel_debug,
                f"{sender}: {output['title']} {output_str} ({output['rating']}, written on {output['createdAt']} by {', '.join(output3)} with {output['comments']} comments) - {output['url']}"
            )
        except IndexError:
            send_message(channel_debug, f'{sender}: No results found!')
            log.warning(
                f'{sender} tried to use the search command in channel {channel_debug} but got no results'
            )
            return
        except Exception as e:
        ...
        if checkperms(full_host, sender):
            ...
            handle.flush()
            log.info(
                f'Raw command used by {sender} in channel {channel_debug}. {args}'
            )
        else:
            send_message(
                channel_debug,
            ...
            log.warning(
                f'{sender} tried to use the raw command in channel {channel_debug}'
            )
        ...
        if checkperms(full_host, sender):
            ...
            handle.flush()
            log.info(
                f'Reboot command used by {sender} in channel {channel_debug}')
            ...
            log.warning(
                f'{sender} tried to use the reboot command in channel {channel_debug}'
            )
        ...
        try:
            ...
            if output["authorPageUrl"] != "":
                send_message(
                    channel_debug,
                    f"{sender}: {output['name']} ({output['authorPageTitle']} - {output['authorPageUrl']}) has {output['pageCount']} pages ({output['pageCountScp']} SCPs, {output['pageCountTale']} Tales, {output['pageCountGoiFormat']} GOI formats, {output['pageCountArtwork']} Artworks, and {output['pageCountOther']} others) with a total rating of {output['totalRating']} and an average rating of {output['meanRating']}. Their latest page is {output['lastPageTitle']} with a rating of {output['lastPageRating']} - {output['lastPageUrl']}"
                )
            else:
                send_message(
                    channel_debug,
                    f"{sender}: {output['name']} has {output['pageCount']} pages ({output['pageCountScp']} SCPs, {output['pageCountTale']} Tales, {output['pageCountGoiFormat']} GOI formats, {output['pageCountArtwork']} Artworks, and {output['pageCountOther']} others) with a total rating of {output['totalRating']} and an average rating of {output['meanRating']}. Their latest page is {output['lastPageTitle']} with a rating of {output['lastPageRating']} - {output['lastPageUrl']}"
                )
        except IndexError:
            send_message(channel_debug, f'{sender}: No results found!')
            log.warning(
                f'{sender} tried to use the author command in channel {channel_debug} but got no results'
            )
            return
        except Exception as e:
            send_message(channel_debug, f'{sender}: Error! String: {e}')
        ...
        for x in range(0, int(latest_range)):
            try:
                ...
                send_message(channel_debug, f'{sender}: No results found!')
                log.warning(
                    f'{sender} tried to use the latest command in channel {channel_debug} but got no results'
                )
                return
            except Exception as e:
                send_message(channel_debug, f'{sender}: Error! String: {e}')
        ...
        send_message(channel_debug,
                     f'{sender}: https://www.rfc-editor.org/rfc/rfc1459.html')
        ...
        output.append(
            f"http://youtube.com/watch?v={json.loads(results)['videos'][0]['id']}"
        )  #0
        ...
        send_message(
            channel_debug,
            f"{sender}: {output[1]} by {output[2]} {output[5]} - length {output[3]} - {output[4]} - {output[0]}"
        )
        ...
        try:
            ...
            if output["authorPageUrl"] != "":
                send_message(
                    channel_debug,
                    f"{sender}: {output['name']} ({output['authorPageTitle']} - {output['authorPageUrl']}) has {output['pageCount']} pages ({output['pageCountLevel']} Levels, {output['pageCountEntity']} Entities, {output['pageCountObject']} Objects, and {output['pageCountOther']} others) with a total rating of {output['totalRating']} and an average rating of {output['meanRating']}. Their latest page is {output['lastPageTitle']} with a rating of {output['lastPageRating']} - {output['lastPageUrl']}"
                )
            else:
                send_message(
                    channel_debug,
                    f"{sender}: {output['name']} has {output['pageCount']} pages ({output['pageCountLevel']} Levels, {output['pageCountEntity']} Entities, {output['pageCountObject']} Objects, and {output['pageCountOther']} others) with a total rating of {output['totalRating']} and an average rating of {output['meanRating']}. Their latest page is {output['lastPageTitle']} with a rating of {output['lastPageRating']} - {output['lastPageUrl']}"
                )
        except IndexError:
            send_message(channel_debug, f'{sender}: No results found!')
            log.warning(
                f'{sender} tried to use the brauthor command in channel {channel_debug} but got no results'
            )
            return
        except Exception as e:
            send_message(channel_debug, f'{sender}: Error! String: {e}')
        ...
        try:
            ...
            send_message(
                channel_debug,
                f"{sender}: {output['title']} ({output['rating']}, written on {output['createdAt']} by {', '.join(output3)} with {output['comments']} comments) - {output['url']}"
            )
        except IndexError:
            send_message(channel_debug, f'{sender}: No results found!')
            log.warning(
                f'{sender} tried to use the brsearch command in channel {channel_debug} but got no results'
            )
            return
        except Exception as e:
            send_message(channel_debug, f'{sender}: Error! String: {e}')
        ...
        log.warning(
            f'{sender} used the ping2 command in channel {channel_debug}')
        log.debug('^ that shouldnt happen')
    elif command == "diagnose":
        log.warning(
            f'{sender} used the diagnose command in channel {channel_debug}')
        # pinging the server
        handle.write(f'PING {server} irc.scpwiki.com :ping\r\n')
        handle.flush()
        if "PONG" in handle.readline().strip():
            send_message(channel_debug,
                         f'{sender}: confirmed connection with IRC server')
        else:
            send_message(channel_debug,
                         f'{sender}: connection with IRC server failed')
            log.error(f'{sender} failed to connect to IRC server')
        # check crom
        # ignore the error
        if crom.wikisearch("SCP-049")["title"] == "SCP-049":
            send_message(channel_debug,
                         f'{sender}: confirmed connection with Crom API')
        else:
            send_message(channel_debug,
                         f'{sender}: connection with Crom API failed')
        ...
        if output == 200:
            send_message(
                channel_debug,
                f'{sender}: confirmed connection with Wikidot ({output})')
        else:
            send_message(
                channel_debug,
                f'{sender}: connection with Wikidot failed. Error code: {output}'
            )
            log.error(
                f'{sender} failed to connect to Wikidot. Error code: {output}')
        output = ""
    elif command == "logs":
        ...
        user_key = pastebin2.generate_user_key(pastebin2.api_dev_key,
                                               pastebin2.username,
                                               pastebin2.password)
        log.info(f"User key generated: {user_key}")
        result = pastebin2.upload_paste(pastebin2.api_dev_key, user_key,
                                        content.strip(), "Mando Logs", "text",
                                        0, "1D")
        ...
        if checkperms(full_host, sender):
            ...
            send_message(
                channel_debug,
                f"{sender}: Automatic Cache stopped (reload with !l)")
        else:
            send_message(
                channel_debug,
                'Sorry, you are not authorized to use this command. Action logged.'
        ...
        if checkperms(full_host, sender):
            ...
            log.warning(
                f'{sender} tried to use the cycle command in channel {channel_debug}'
            )
    elif command == "!kick":
        if checkperms(full_host, sender):
            try:
            ...
            if len(args[0]) == 0:
                send_message(
                    channel_debug,
                    f'{sender}: Invalid format. Use !kick <nick> <?reason>')
            elif len(args[1]) == 0:
                handle.write(
                    f'KICK {channel_debug} {args[0]} :Kicked by {sender}\r\n')
                if "482" in handle.readline().strip():
                    rand = r.randint(0, 10)
                    if rand == 0:
                        send_message(channel_debug,
                                     f'{sender}: i cant kick them ;-;')
                    else:
                        send_message(
                            channel_debug,
                            f'{sender}: I am not authorizd to kick {args[0]}.')
                    log.warning(
                        f'{sender} tried to kick {args[0]} from {channel_debug} but is not authorized'
                    )
                else:
                ...
                handle.flush()
                log.info(
                    f'{sender} kicked {args[0]} from {channel_debug} for {args[1]}'
                )
        else:
            send_message(
                channel_debug,
            ...
            log.warning(
                f'{sender} tried to use the kick command in channel {channel_debug}'
            )
    elif command == "!ban":
        if checkperms(full_host, sender):
            ...
            if len(args[0]) == 0:
                send_message(
                    channel_debug,
                    f'{sender}: Invalid format. Use !ban <nick> <?reason>')
            elif len(args[1]) == 0:
                handle.write(
                    f'MODE {channel_debug} +b {args[0]} :Banned by {sender}\r\n'
                )
                handle.flush()
                log.info(f'{sender} banned {args[0]} from {channel_debug}')
            else:
                handle.write(
                    f'MODE {channel_debug} +b {args[0]} :{args[1]}\r\n')
                handle.flush()
                log.info(
                    f'{sender} banned {args[0]} from {channel_debug} for {args[1]}'
                )
                # add duration later
        else:
            send_message(
            ...
            log.warning(
                f'{sender} tried to use the ban command in channel {channel_debug}'
            )
...
if __name__ == "__main__":
    try:
        ...
        handle = ircsock.makefile(mode='rw',
                                  buffering=1,
                                  encoding='utf-8',
                                  newline='\r\n')
        ...
        while True:
            ...
            chanserv_halfop = (":ChanServ!ChanServ@conflict.irc.scpwiki.com"
                               in line and "MODE" in line and "+h" in line
                               and nick in line and "#site22" in line)
            ...
            if chanserv_halfop and history_bypass == 0:
                ...
                if chanserv_halfop:
                    ...
                    handle.write(
                        'PRIVMSG #cheesepoop9870 :History cleared!\r\n')
                    handle.flush()
                ...
                if len(msg_parts) > 1:
                    ...
                    if history_bypass == 1:
                        if channel_temp == nick:
                            channel_temp = sender
                        log.info(
                            f"Command sent: {command} {args} ({sender} -> {channel_temp})"
                        )
                        handle_command(command, args, handle, sender,
                                       channel_temp, full_host)
    except Exception as e:
        print(f"Error: {e}")
